...
@Component({
  ...
  templateUrl: './livis-login.component.html',
  styleUrls: ['./livis-login.component.css'],
...
export class LivisLoginComponent implements OnInit, OnDestroy {
  ...
  loginUser(event) {
    ...
    this.authenticationService.userLogin(this.loginForm.get('email').value,this.loginForm.get('password').value,this.loginForm.get('workstation_name').value)
        .subscribe(data =>{
          // console.log("hi",data);
