...
@singleton
class Pillar_part():
	def __init__(self, num_classes = 8, img_size = (1280, 600), min_threshold = PILLAR_PART_MODEL_THRESHOLD):
	...
	def inference(self, input_frame):
		...
		for features, count in self.predictions_two.items():
			if (features != "Clip_Absence") and (features != "Felt_Absence"):
...
@singleton
class Roof_part():
	def __init__(self, num_classes = 8, img_size = (1280, 600), min_threshold = ROOF_PART_MODEL_THRESHOLD):
		self.num_classes = num_classes
	...
	def inference(self, input_frame):
		...
		for features, count in self.predictions_two.items():
			if (features != "Clip_Absence") and (features != "Felt_Absence") and (features != "Black_Clip_Absence"):
				self.predictions["feature_name"] = features
				self.predictions["count"] = count
				pred = self.predictions.copy()
				self.predicted_taco.append(pred	)
...
class Pillar_part_presence_abs():
	def __init__(self, num_classes = 2, img_size = (1280, 720), min_threshold = PILLAR_PART_PRESENCE_ABSENCE_THRESHOLD):
		self.num_classes = num_classes
