...
def create_temp_folder(directory):
    directory = "C:\\Users\\urahu\\Desktop\\livis_jun26_copy\\livis\\backend\\LAMOZO\\lamozo\\livis\\cataler_6_7\\model_no"
...
def list_video_stitch(opt,final_frame_list,mask,part_id):# inserting plugged cell mask in final frame
    outvid = cv2.VideoWriter(str(part_id)+'.mp4',cv2.VideoWriter_fourcc('M','J','P','G'), 30, (1280,960))
    plug_seg = cv2.resize(mask,(640,480))
    # vidObj = cv2.VideoCapture(vid_path) 
    for final_frame in  final_frame_list:	
        final_frame[480:,640:,:] = plug_seg 
        
        outvid.write(final_frame)

def list_temp_vid(opt,final_frame_list):# list to video
    print(len(final_frame_list))
    outvid = cv2.VideoWriter(opt.temp_vid_path,cv2.VideoWriter_fourcc('M','J','P','G'), 30, (1280,960))
    # plug_seg = cv2.resize(mask,(640,480))
    # vidObj = cv2.VideoCapture(vid_path) 
    for final_frame in  final_frame_list:	
        ...
        outvid.write(final_frame)
    print('Temporary video writing done!!')
    

def video_stitch(opt,part_id):# inserting plugged cell mask in final frame
    mask = cv2.imread(opt.mask_path)
    outvid = cv2.VideoWriter(os.path.join(opt.video_records, str(part_id)+'.mp4'),cv2.VideoWriter_fourcc('M','J','P','G'), 30, (1280,960))
    plug_seg = cv2.resize(mask,(640,480))
    vidobj = cv2.VideoCapture(opt.temp_vid_path1) 
    frame_count = vidobj.get(cv2.CAP_PROP_FRAME_COUNT)
    ret,frame = vidobj.read()
    for i in range(int(frame_count)):	
        frame[480:,640:,:] = plug_seg 
        outvid.write(frame)
    print('Video stitching done!!!')

def video_stitch_final(opt,final_frame_list,seg_mask,part_id):
    i = 0
    # create_folder(opt.video_records)
    outvid = cv2.VideoWriter(os.path.join(opt.video_records, str(part_id)+'.mp4'),cv2.VideoWriter_fourcc('M','J','P','G'), 30, (1280,960))
    # print(os.path.join(opt.video_records, str(part_id)+'.mp4'))
    while i < (len(final_frame_list)-3):
        concated_frame = cv2.vconcat([cv2.hconcat([final_frame_list[i],final_frame_list[i+1]]),cv2.hconcat([final_frame_list[i+2],seg_mask])])  
        i += 3
        # print(concated_frame.shape)
        outvid.write(concated_frame)
    # print('Video stitching done!!!')
...
class CacheHelperWorkstation():
    try:
        def __init__(self, CLIENT_ID, CLIENT_PORT):
            self.redis_cache = redis.StrictRedis(host=CLIENT_ID, port=CLIENT_PORT, db=0, socket_timeout=1)
            print("REDIS CACHE UP!")
