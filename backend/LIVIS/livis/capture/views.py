...
@csrf_exempt
@permission_classes((AllowAny,))
def consumer_video_input(data):
    data = json.loads(data.body)
    message, status_code = start_consumer_video_stream(data)
    if status_code == 200:
...
def consumer_camera_preview(request,wid,cameraname):
    return StreamingHttpResponse(start_consumer_camera_preview(wid,cameraname), content_type='multipart/x-mixed-replace; boundary=frame')
    #data = json.loads(data.body)
    #message, status_code = start_consumer_camera_preview(wid,cameraname)
    #if status_code == 200:
    #    return HttpResponse(json.dumps({'Message': 'Success!', 'data': message}), content_type="application/json")
    #else:
    #    return HttpResponse(json.dumps({'Message': 'fail!', 'data': message}), content_type="application/json")
