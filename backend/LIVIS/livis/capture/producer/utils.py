    ...
    def stream_bulk_upload_data(self, compressed_file_path, archive_format, part_id, workstation_id):
        ...
        for filename in os.listdir(extracted_img_path):
            ...
            while i <= len(img_str)-1:
                ...
                payload = {"frame": buf_str, "part_id": str(part_id), "frame_idx": frames_iter}
                self.obj.send(self.topic, value=payload)
                time.sleep(1)
                i = i+10000
            payload = {"frame": "END", "part_id": part_id, "workstation_id": workstation_id, "frames_iter": frames_iter}
            self.obj.send(self.topic, value=payload)
        ...
        while (cap.isOpened()):
             ...
             if ret == True:
                 ...
                 with open("tmp.jpg", 'rb') as f:
                     im_b64 = base64.b64encode(f.read())
                 payload_video_frame = {"frame": str(im_b64), "part_id": part_id,
                                        "workstation_id": workstation_id, "frames_iter": frames_iter}
...
def start_video_stream(data):
    ...
    for id in camera_indexes:
        ...
        status_code = producer_ws.stream_video(part_id, workstation_id, id)
        logging.info('Done streaming')
        message = "Done streaming"
        status_code = 200
        return message, status_code
